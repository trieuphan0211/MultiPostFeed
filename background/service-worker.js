...
async function showFacebookTab(tabId) {
  ...
  if (state.tabId) {
    try {
      ...
      if (tab && /facebook\.com/.test(tab.url || "")) {
        await chrome.tabs.update(state.tabId, { url, active: true, autoDiscardable: false });
        if (tab.windowId) {
          await chrome.windows.update(tab.windowId, { focused: true }).catch(() => {});
        }
